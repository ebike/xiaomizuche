...
public class JCClient implements ITCPConnectionListener
{
	...
	{
		Log.d(TAG, "Send:  " + protocol.getDataType().toString() + "! _isLogined = " + String.valueOf(_isLogined));
	...
	{
		_isLogining = false;
		Log.d(TAG, "Recv data length:" + String.valueOf(dataLength));
		if (_sendTimer != null)
		{
			synchronized(_tasksLock)
			{
		...
		Log.d(TAG, "Recv: " + protocol.getDataType().toString());
