...
import com.xiaomizuche.bean.SendDataModel;
import com.xiaomizuche.client.IJCProtocolScanner.ScanResult;
...
public class TCPConnection {
    ...
    public void init(Context context, String site, int port) {
        Log.d(TAG, "init()!site = " + site + "! port = " + String.valueOf(port));
        if (!_isInited) {
            _context = context;
            _site = site;
            _port = port;
    ...
    private void onRecvData(byte[] data, int offset, int length) {
        ...
        if (_listener != null) {
            if (length > ONE_PROTOCOL_BUFFER_LENGTH) {
                //收到的数据有错，不做其它处理
                Log.e(TAG, "The data is too long!");
                return;
            }
                    ...
                    try {
                        ...
                        synchronized (_streamLock) {
                            if (_os != null) {
                                ...
                                Log.d(TAG, "Send Data! Buffer length: " + String.valueOf(buffer.length));
                                _os.write(buffer);
    ...
    public class RecvThread extends Thread {
            ...
            while (this.isAlive()) {
                try {
                    if (_is != null) {
                        ...
                        Log.d(TAG, "Before read! offset = " + String.valueOf(_bufferPos) + "! length = "
                        ...
                        Log.d(TAG, "Recv Data! length = " + String.valueOf(readLength));
                        if (readLength == -1) {
                            //连接断开
                            disconnect();
                            onConnectFailed();
        ...
        private ScanResult preparseProcess(int totalLength, int length) {
            Log.d(TAG, "preparseProcess : length = " + String.valueOf(length)
            ...
            Log.d(TAG, "Scan result: " + result.getEnumScanResult().toString() + "! unparsedPos = " + String.valueOf(result.getUnprocessedLocation()));
