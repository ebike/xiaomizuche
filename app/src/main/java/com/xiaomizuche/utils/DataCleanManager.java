...
//        cleanSharedPreference(context);
    ...
    public static String getTotalSize(Context context) throws Exception {
        long internalCache = getFolderSize(context.getCacheDir());
        long externalCache = 0;
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            externalCache = getFolderSize(context.getCacheDir());
        }
//        long databases = getFolderSize(new File("/data/data/" + context.getPackageName() + "/databases"));
//        long sharedPreference = getFolderSize(new File("/data/data/" + context.getPackageName() + "/shared_prefs"));
        long files = getFolderSize(context.getFilesDir());
        long total = internalCache + externalCache + files;
        if (total != 0) {
